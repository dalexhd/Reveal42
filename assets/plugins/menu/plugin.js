...
function onDocumentKeyDown(event) {
  ...
  if (isOpen()) {
    ...
    switch (event.keyCode) {
      ...
      // home
      case 36:
        selectAll(".active-menu-panel .slide-menu-items li").forEach(function (
          item
        ) {
          item.classList.remove("selected");
        });
...
function prevPanel() {
  var next =
    parseInt(select(".active-toolbar-button").getAttribute("data-button")) - 1;
  if (next < 0) {
    next = buttons - 1;
  }
  openPanel(
...
function initMenu() {
  if (!initialised) {
    ...
    if (typeof options.width === "string") {
      if (
        ["normal", "wide", "third", "half", "full"].indexOf(options.width) != -1
      ) {
