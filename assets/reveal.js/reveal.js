...
export default function (revealElement, options) {
  ...
  function getComputedSlideSize(presentationWidth, presentationHeight) {
    ...
    if (typeof size.width === "string" && size.width.endsWith("%")) {
      size.width = (parseInt(size.width, 10) / 100) * size.presentationWidth;
    }

    // Slide height may be a percentage of available height
    if (typeof size.height === "string" && size.height.endsWith("%")) {
  ...
  function slide(h, v, f, o) {
    ...
    const currentVerticalSlides = currentHorizontalSlide.querySelectorAll(
      "section"
    );
  ...
  function updateSlides(selector, index) {
    ...
    const printMode = print.isPrintingPDF();
  ...
  function getProgress() {
    ...
    if (currentSlide) {
      ...
      if (allFragments.length > 0) {
        const visibleFragments = currentSlide.querySelectorAll(
          ".fragment.visible"
        );
